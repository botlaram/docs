...
These components are responsible for managing the cluster and ensuring the desired state of the system.
...
Node components manage the execution of workloads on individual machines.
...
These are additional features often deployed in a Kubernetes cluster.
...
Container Runtime
Kubernetes supports multiple container runtimes. Ensure one of these is installed and configured:
...
6. Installing Kubernetes with kubeadm
...
Communication between the control plane and worker nodes is critical.
Required ports:
...
Worker nodes communicate with the control plane and other nodes to manage workloads.
Required ports:
...
a) Pod Network CIDR
Kubernetes clusters use a Pod network CIDR range to assign IPs to Pods. For example:
...
b) Service CIDR
Kubernetes services are assigned virtual IPs from a separate CIDR range. For example:
...
Kubernetes itself does not handle networking; it relies on network plugins conforming to the Container Network Interface (CNI) specification. Some popular CNI plugins include:
...
Apply the CNI plugin’s YAML manifest after initializing the cluster. Example for Flannel:
bash
Copy code
kubectl apply -f https://raw.githubusercontent.com/coreos/flannel/master/Documentation/kube-flannel.yml
...
Firewall Rules
Ensure firewall rules allow traffic on the required ports for Kubernetes components and networking.
Allow traffic between the Pod CIDR range, Service CIDR range, and nodes.
Cloud-Specific Networking
If running Kubernetes in a cloud environment, configure networking based on the cloud provider’s requirements. Most managed Kubernetes services (like AWS EKS, Azure AKS, or GKE) have native integrations.
Network Policies
Network policies define rules for traffic flow between Pods and services.
Use tools like Calico or Cilium to enforce these policies.
7. Verifying Network Connectivity
Ping Test Between Pods:
Deploy Pods on different nodes and verify they can communicate:
...
## what different phased k8 execute while running kubeadm init
provide in short points

4/4

ChatGPT said:
ChatGPT
When you run kubeadm init, it executes a series of phases to set up a Kubernetes control plane. Each phase performs a specific task to bootstrap the cluster. Here's a summary:
...
Deploys static Pods for:
kube-apiserver
kube-controller-manager
kube-scheduler
Manages these via the kubelet.
8. Bootstrap Token
...
[github](https://github.com/sandervanvugt/cka.git)
